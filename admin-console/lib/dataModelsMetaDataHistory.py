"""Pane for Admin Console ribbon destination: Data->Models->MetaData History.

This pane presents details of model metadata in the archive schema.
"""
...
class DataPanel(wx.Panel):
	def __init__(self, parent, log, metaDataSnapshots):
		try:
	...
	def loadData(self, selection):
		thisSnapshot = self.metaDataSnapshots.get(selection)
		self.logger.debug('DataPanel: loadData: {}'.format(thisSnapshot))
		self.modelInstance = thisSnapshot.get('meta_data')
		...
		modelHeader = wx.StaticText(self, wx.ID_ANY, 'MetaData Snapshots:')
...
class SnapshotListCtrlPanel(wx.Panel, listmix.ColumnSorterMixin):
	def __init__(self, parent, log, dataPanelRef, metaDataSnapshots):
	...
	def getFormattedSnapshots(self):
		objectId = 0
		for timeStamp in self.metaDataSnapshots:
...
class Main():
	def __init__(self, thisPanel, log, api):
		...
		self.ModelHistoryBox = wx.StaticBox(self.thisPanel, wx.ID_ANY, 'MetaData History')
		self.modelType = None
		self.modelTypes = []
		self.model = {}
		...
		self.dataPanel = DataPanel(self.rawDataPanel, self.logger, self.metaDataSnapshots)
	...
	def getMetaDataSnapshots(self, identifier):
		apiResults = self.api.getResource('archive/modelMetadataSnapshot/{}'.format(identifier))
		self.metaDataSnapshots.clear()
		for snapshotId, snapshotData in apiResults.items():
			simpleDateString = snapshotData.get('time_stamp')
			self.metaDataSnapshots[simpleDateString] = snapshotData
	...
	def EvtChooseModel(self, event):
		...
		wx.BeginBusyCursor()
		self.getMetaDataSnapshots(modelId)
		if self.snapshotList is not None:
			self.snapshotList.Destroy()
		self.snapshotList = SnapshotListCtrlPanel(self.rawSnapshotPanel, self.logger, self, self.metaDataSnapshots)
