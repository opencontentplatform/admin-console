"""Pane for Admin Console ribbon destination: Data->Models->Update MetaData.
...
class UpdateForm(ScrolledPanel):
	def __init__(self, parent, logger, api, apps, appsWithMetaData, metadata, classDefinitions):
		...
		## Description section
		text = 'Select the Application and current metadata object to update, make the desired updates, and click Submit to save.'
	...
	def constructTargetObject(self, vbox):
		...
		self.discoverableObjectClass = wx.TextCtrl(targetRefPanel, wx.ID_ANY, style=wx.TE_READONLY)
		targetPanelSizer.Add(thisText, 0, wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL)
		targetPanelSizer.Add(self.discoverableObjectClass, 0, wx.EXPAND)
		...
		vbox.AddSpacer(20)
		target_collapsible_panel.Expand()
	...
	def populateFormWithMetaData(self):
		...
		## Tier 3 Group:
		##   App -> Environment
		self.envClassChoice.Clear()
		self.envClassChoice.AppendItems(classList)
		...
		for entry in classList:
			if self.envClassName == entry:
				self.envClassChoice.SetSelection(entryId)
				break
			entryId += 1
		...
		entryId = 0
		for entry in envClassAttrList:
			if self.envAttr == entry:
				self.envAttrChoice.SetSelection(entryId)
				break
			entryId += 1

		self.envPanel.Clear()
		for key,value in self.metaData.get('tier3_match_patterns', {}).items():
			self.envPatterns[key] = value
		...
		## Tier 1 Group (bottom logical level):
		##   App -> Environment -> Software -> Location
		self.locClassChoice.Clear()
		self.locClassChoice.AppendItems(classList)
		...
		entryId = 0
		for entry in classList:
			if self.locClassName == entry:
				self.locClassChoice.SetSelection(entryId)
				break
			entryId += 1
		...
		entryId = 0
		for entry in locClassAttrList:
			if self.locAttr == entry:
				self.locAttrChoice.SetSelection(entryId)
				break
			entryId += 1
		self.locPanel.Clear()
		for key,value in self.metaData.get('tier1_match_patterns', {}).items():
			self.locPatterns[key] = value
		...
		self.targetAttrChoice.AppendItems(classList)
		...
		self.targetAttr = self.metaData.get('target_match_attribute')
		...
		self.targetAttrChoice.SetSelection(0)
		entryId = 0
		for entry in targetClassAttrList:
			if self.targetAttr == entry:
				self.targetAttrChoice.SetSelection(entryId)
				break
			entryId += 1
	...
	def onInsert(self, event=None):
		...
		self.logger.info('-'*40)
		#self.logger.info('{}'.format(self.app))
		...
		self.logger.info('-'*40)
		self.logger.info('name: {}'.format(self.groupCtrlName.GetValue()))
		attributes['tier2_name'] = self.groupCtrlName.GetValue()
		#attributes['tier2_qualifier'] = ''
		...
		self.updateMetaData(attributes)

		## end onInsert
		return

	def updateMetaData(self, attributes):
		self.logger.debug('updateMetaData: value == OK')
		wx.BeginBusyCursor()
		content = {}
		content['source'] = 'admin console'
		content['data'] = attributes
		(responseCode, responseAsJson) = self.api.putResource('data/ModelMetaData/{}'.format(self.metaDataId), {'content': content})
		wx.EndBusyCursor()
		if responseCode == 200:
			...
			dlgResult.ShowModal()
			dlgResult.Destroy()
			self.logger.debug('updateMetaData: metadata added.')
		else:
			errorMsg = json.dumps(responseAsJson)
			with suppress(Exception):
				errorMsg = str(responseAsJson[list(responseAsJson.keys())[0]])
